...
if [ "$POWERLINE_RIGHT_B" = "" ]; then
  ...
  POWERLINE_RIGHT_B=%(?."$POWERLINE_DATE_FORMAT".%F{red}✘ %?)
elif [ "$POWERLINE_RIGHT_B" = "exit-status" ]; then
  POWERLINE_RIGHT_B=%(?.%F{green}✔ %?.%F{red}✘ %?)
...
POWERLINE_GIT_INFO_RIGHT="%F{240}"$'\u2b82'"%F{black}%K{240}"$'$(git_prompt_info)'" %f"
